// F50 · Wave 1 · duplicado de QuotesPage.tsx para v2. V1 queda intocada.
// Cambios v2 respecto de v1:
//   · Wave 1.a (parcial) · toast success post-submit vía useToast del AuthToast.
//     El toast de error de auto-guardado (Wave 1.a completa) queda deferido
//     hasta que se apruebe explícitamente un refactor del QuoteContext (que
//     hoy no reporta las fallas de saveDrafts).
//   · Wave 1.c · pencil inline en la tarjeta del borrador + edit-in-place
//     (Enter confirma, Escape cancela, blur también confirma). Reutiliza
//     renameDraft del context que ya existía.
//
// La lógica de negocio (drafts, submit, delete, items list, buyer info)
// es idéntica a QuotesPage.tsx. Si aparece un bug en la lógica compartida,
// hay que fixear en ambos archivos.
...
interface QuotesPageV2Props {
    /** Callback al cerrar la page · vuelve a Catalog */
    onBack?: () => void
}

export default function QuotesPageV2({ onBack }: QuotesPageV2Props) {
    const {
        activeDrafts, submittedDrafts, activeDraftId, activeDraft,
        buyerInfo, setActiveDraft, createDraft, deleteDraft, removeItem,
        updateItem, submitDraft, startEditingItem, quotedHistory, renameDraft,
    } = useQuote()
    ...
    // F50 · Wave 1.a (parcial) · toast system.
    ...
    const handleSubmit = () => {
        ...
        setSubmittedRef(ref)
        // F50 · Wave 1.a · confirmación no-disruptiva. Coexiste con el overlay
        // full-screen actual — cuando el overlay se retire en una wave futura,
        // este toast queda como feedback principal.
        addToast('success', `Selection submitted · Ref ${ref}`)
    }
    ...
    if (submittedRef) {
        return (
            <>
                <SubmittedConfirmation refNumber={submittedRef} tenantName={buyerInfo.tenant.name} onDone={() => { setSubmittedRef(null); setSection('submitted') }} onBack={onBack} />
                    ...
                    {visibleDrafts.length === 0 ? (
                        ...
                        visibleDrafts.map(d => (
                            <DraftListItem
                                ...
                                canRename={section === 'drafts'}
                                onRename={(name) => renameDraft(d.id, name)}
                            />
                        ))
                    )}
                </aside>
...
/* ─── Sub-components (locales al módulo · no colisionan con QuotesPage.tsx) ─ */
...
interface DraftListItemProps {
    ...
    /** F50 · Wave 1.c · v2 · gate del rename inline. Solo true en la pestaña
     *  "Drafts" (los submitted son read-only). */
    canRename?: boolean
    onRename?: (name: string) => void
}

function DraftListItem({ draft, selected, onClick, onDelete, confirmDelete, onCancelDelete, onConfirmDelete, canRename = false, onRename }: DraftListItemProps) {
    const itemCount = draft.items.length
    const unitCount = draft.items.reduce((s, it) => s + it.qty, 0)
    const total = draft.items.reduce((s, it) => s + it.totalPrice, 0)

    // F50 · Wave 1.c · state del inline rename.
    ...
    const commitRename = () => {
        ...
        if (trimmed && trimmed !== draft.name && onRename) {
            onRename(trimmed)
        }
        setIsEditing(false)
    }
            ...
            >
                ...
                {isEditing && canRename ? (
                    <input
                        ...
                        className="mt-0.5 w-full rounded border border-primary bg-background px-1.5 py-0.5 text-sm font-bold text-foreground outline-none ring-2 ring-primary/20"
                        maxLength={80}
                    />
                ) : (
                    <div className="mt-0.5 flex items-center gap-1.5 pr-6">
                        <span className="truncate text-sm font-bold text-foreground">{draft.name}</span>
                        {canRename && onRename && (
                            <span
                                role="button"
                                tabIndex={0}
                                ...
                                title="Rename"
                                aria-label="Rename draft"
                                className="inline-flex h-5 w-5 shrink-0 items-center justify-center rounded text-muted-foreground opacity-0 transition-all hover:bg-primary/10 hover:text-primary group-hover:opacity-100 focus:opacity-100"
                            >
...
/** view mode del listing · persist localStorage por session (no per-draft). */
type ViewMode = 'flat' | 'byspace'
const VIEW_MODE_KEY = 'expert-hub-quotes-view-mode'

/** agrupa items por settingCode · idéntico a QuotesPage.tsx */
