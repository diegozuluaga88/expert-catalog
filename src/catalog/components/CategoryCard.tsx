...
function PhotoVariant({ category }: { category: Category }) {
  ...
  // placeholder gradient neutro. Diego confirmó (2026-07-10) que las
  // category cards SIEMPRE son imagen · el video vive en el hero del brand.
  const src = category.cardImage ?? category.products?.[0]?.images?.[0]

  if (!src) {
    return <div className="absolute inset-0 bg-gradient-to-br from-muted via-muted to-primary/20" />
  }
  return (
    <img
      src={src}
