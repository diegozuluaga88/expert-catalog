// Phase 2 Fix #5 — Product detail modal · centered + tabs (post Diego feedback).
// Diego pidió consistency con el resto de modales del expert-hub (Dialog centrado tipo
// CatalogImportModal) en vez de side-panel. Mantenemos tabs PERO con contenido
// integrado · no 1 paragraph per tab. Layout 65/35 top con gallery + buy box.
//
// Tabs (4 vs 5 originales · integrados):
//  - Overview · description + standard features + key specs preview
//  - Variants & Materials · finishes + fabric options + material tiers · cost callouts
//  - Specifications · specs + performance combined (eran 2 tabs redundantes)
//  - Resources · cleaning + documents combined
//
// Discontinued state · banner top + disable Add to Quote (Diego's separate ask).
...
type DetailTab = 'overview' | 'variants' | 'specs' | 'resources'
...
export default function ProductDetailPanel({
    ...
    // Buy box state · per-line variant selections
    ...
    // Reset state al cambiar de product
    useEffect(() => {
        if (product) {
            setQty(1)
                ...
                <div className="fixed inset-0 flex items-center justify-center p-4">
                    ...
                    >
                        <Dialog.Panel className="relative flex max-h-[92vh] w-full max-w-6xl flex-col overflow-hidden rounded-2xl border border-border bg-card shadow-2xl">
                            ...
                            {/* Discontinued banner · stakeholder ask · block actions visually */}
                            {isDiscontinued && (
                                <div className="flex items-center gap-3 border-b border-border bg-muted/80 px-6 py-3 text-sm">
                                    <Ban className="h-5 w-5 flex-shrink-0 text-muted-foreground" />
                                    <div className="flex-1">
                                        <span className="font-semibold text-foreground">This product is discontinued.</span>
                                        <span className="ml-1 text-muted-foreground">
                                            We're keeping it visible for historical reference only · quoting is disabled. Use Substitute to find an alternative.
                                        </span>
                                    </div>
                                </div>
                            )}
                            {isDiscrepancy && (
                                <div className="flex items-center gap-3 border-b border-border bg-amber-500/10 px-6 py-3 text-sm">
                                    <Sparkles className="h-5 w-5 flex-shrink-0 text-amber-700 dark:text-amber-400" />
                                    <span className="flex-1 text-foreground">
                                        <span className="font-semibold">Catalog out of sync.</span>
                                        <span className="ml-1 text-muted-foreground">
                                            Some attributes may be stale · sync the {product.brand} catalog from Manage Catalogs for the latest data.
                                        </span>
                                    </span>
                                </div>
                            )}
                            ...
                            <div className="flex-1 overflow-y-auto">
                                {/* Hero · 2 cols 65/35 · gallery + buy box */}
                                <div className="grid grid-cols-1 gap-6 p-6 lg:grid-cols-[1.7fr_1fr]">
                                    <div>
                                ...
                                {/* Tab nav · maximiza info en pocos clicks */}
                                <div className="sticky top-0 z-[1] border-y border-border bg-muted/30">
                                    <div className="flex gap-1 px-6" role="tablist" aria-label="Product details">
                                ...
                                {/* Active tab content · integrates múltiples sections per tab */}
...
function SkuChip({ label, value, copied, onCopy }: { label: string; value: string; copied: boolean; onCopy: () => void }) {
    return (
        <button
            type="button"
            onClick={onCopy}
            className="group inline-flex items-center gap-1.5 rounded-md border border-border bg-muted/40 px-2 py-1 font-mono text-foreground transition-colors hover:bg-muted"
    ...
    return (
        <div className="mt-4">
            <h3 className="mb-2 text-xs font-semibold uppercase tracking-wide text-foreground">
                ...
                {selectedCw && (
                    <span className="ml-2 font-normal normal-case tracking-normal text-muted-foreground">· {selectedCw.name}</span>
                )}
            </h3>
...
function BuyBox({ product, qty, setQty, totals, savingsPct, disabled, onAddToQuote }: BuyBoxProps) {
    ...
    return (
        <div className="rounded-xl border border-border bg-background p-4 shadow-sm">
            <div className="mb-3">
                ...
                {savingsPct > 0 && (
                    <span
                        className="mt-1 inline-flex items-center rounded-full bg-amber-500/15 px-2 py-0.5 text-[11px] font-bold text-amber-700 dark:text-amber-400"
                        title={`Manufacturer list $${product.listPrice?.toLocaleString()} · dealer discount applied`}
                    >
                        Save {savingsPct}%
                    </span>
                )}
                {totals.nextVolumeTier && (
                    <p className="mt-2 text-xs text-foreground">
                        ...
                        <span className="font-bold">${totals.nextVolumeTier.savings.toLocaleString()}</span>
                    </p>
                )}
            </div>

            <div className="mb-3">
                <h4 className="mb-1.5 text-xs font-semibold uppercase tracking-wide text-foreground">Quantity</h4>
                <div className="flex items-center gap-2">
                    <button
                        type="button"
                        onClick={() => setQty(Math.max(1, qty - 1))}
                        disabled={disabled}
                        className="inline-flex h-9 w-9 items-center justify-center rounded-lg border border-input text-foreground transition-colors hover:bg-muted disabled:cursor-not-allowed disabled:opacity-50"
            ...
            <div className="my-3 space-y-1 border-y border-border py-3">
            ...
            <button
                ...
                disabled={disabled}
                title={disabled ? 'This product is discontinued · quoting is disabled' : 'Add to your quote'}
            ...
            >
                {disabled ? <Ban className="h-4 w-4" /> : <>Add to Quote <ArrowUpRight className="h-4 w-4" /></>}
...
/* ─── Tab contents · integrated info ─────────────────────────────── */

function OverviewTab({ product }: { product: Product }) {
    return (
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
            <div>
                <h3 className="mb-2 text-sm font-bold uppercase tracking-wide text-foreground">Description</h3>
                <p className="text-sm leading-relaxed text-muted-foreground">{product.description}</p>

                {product.tags && product.tags.length > 0 && (
                    <div className="mt-4">
                        <h4 className="mb-2 text-xs font-semibold uppercase tracking-wide text-foreground">Tags</h4>
            ...
            <div className="space-y-4">
                {product.standardFeatures && product.standardFeatures.length > 0 && (
                    <div>
                        <h3 className="mb-2 text-sm font-bold uppercase tracking-wide text-foreground">Standard features</h3>
                        <ul className="space-y-1.5 text-sm text-muted-foreground">
                            {product.standardFeatures.map(f => (
                                <li key={f} className="flex gap-2">
                                    <span className="mt-1 inline-block h-1 w-1 flex-shrink-0 rounded-full bg-foreground" />
                                    {f}
                                </li>
                            ))}
                        </ul>
                ...
                {product.optionalFeatures && product.optionalFeatures.length > 0 && (
                    <div>
                        <h3 className="mb-2 text-sm font-bold uppercase tracking-wide text-foreground">Optional features</h3>
                        <ul className="space-y-1.5 text-sm text-muted-foreground">
    ...
    return (
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
            {finishes && finishes.length > 0 && (
                <section>
                    <h3 className="mb-3 text-sm font-bold uppercase tracking-wide text-foreground">Finishes</h3>
            ...
            {fabricOptions && fabricOptions.length > 0 && (
                <section>
                    <div className="mb-3 flex items-center justify-between">
                        <h3 className="text-sm font-bold uppercase tracking-wide text-foreground">Fabric options</h3>
            ...
            {materialTiers && materialTiers.length > 1 && (
                <section>
                    <h3 className="mb-3 text-sm font-bold uppercase tracking-wide text-foreground">Material tier</h3>
            ...
            {(product.material || product.upholstery) && (
                <section>
                    <h3 className="mb-3 text-sm font-bold uppercase tracking-wide text-foreground">Materials info</h3>
                    <dl className="space-y-1.5 text-sm">
                        {product.material && (
                            <div className="flex gap-2">
                                <dt className="font-semibold text-foreground w-24">Material:</dt>
                                <dd className="text-muted-foreground">{product.material}</dd>
                            </div>
                        )}
                        {product.upholstery && (
                            <div className="flex gap-2">
                                <dt className="font-semibold text-foreground w-24">Upholstery:</dt>
                                <dd className="text-muted-foreground">{product.upholstery}</dd>
                            </div>
                        )}
                    </dl>
                </section>
...
function SpecsTab({ product }: { product: Product }) {
    ...
    const hasPerf = Object.keys(product.performance).length > 0
    const hasDim = !!product.dimensions
    return (
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
            {hasSpecs && (
                <div>
                    <h3 className="mb-3 text-sm font-bold uppercase tracking-wide text-foreground">Specifications</h3>
                    <InfoTable data={product.specs} />
                </div>
            )}
            {hasPerf && (
                <div>
                    <h3 className="mb-3 text-sm font-bold uppercase tracking-wide text-foreground">Performance</h3>
                    <InfoTable data={product.performance} />
                </div>
            )}
            {hasDim && product.dimensions && (
                <div>
                    <h3 className="mb-3 text-sm font-bold uppercase tracking-wide text-foreground">Dimensions</h3>
                    <InfoTable data={product.dimensions as unknown as Record<string, string>} />
                </div>
            )}
            {!hasSpecs && !hasPerf && !hasDim && (
...
function ResourcesTab({ product }: { product: Product }) {
    ...
    return (
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
            {hasCleaning && (
                <div>
                    <h3 className="mb-3 text-sm font-bold uppercase tracking-wide text-foreground">Care & Maintenance</h3>
                    <p className="text-sm leading-relaxed text-muted-foreground">{product.cleaning}</p>
                </div>
            )}
            {hasDocs && (
                <div>
                    <h3 className="mb-3 text-sm font-bold uppercase tracking-wide text-foreground">Documents</h3>
            ...
            <tbody>
                {Object.entries(data).map(([key, val]) => (
                    <tr key={key} className="border-b border-border last:border-0">
                        <td className="w-40 py-2 pr-3 align-top text-xs font-semibold uppercase tracking-wide text-foreground">
                            {key}
                        </td>
